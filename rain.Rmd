...
From the table above, we can see that our dependent variable "RainTomorrow" is a **categorical variable** with two unique values "Yes" and "No". Based on our sampled data, there is **79%** of the time that it **will not rain tomorrow** and **21%** of the time that it **will rain tomorrow**. This indicates that the data is imbalanced.

Common approach to deal with imbalance data is to undersample the majority class and oversample the minority class. However, this approach is not implemented as the current imbalanced dataset better reflects the reality, where it is common to have more non-rainy days than rainy days.
...
From the boxplots above, "Rainfall", "WindGustSpeed", "WindSpeed9am", "WindSpeed3pm" and "Humidity9am" **have outliers** in their data. The remaining variables are proven to have no outliers. Before proceeding with model training, these outliers should be treated.
...
```
Based on the table above, the results are mixed, but cross-validated random forest is chosen as a better model among the two. This decision is made on the reason that, in general, optimizing for recall, F1-Score and AUC is more important than precision and accuracy for imbalanced dataset. 

In an imbalanced dataset, where one class is much more prevalent than the other, simply predicting the majority class can lead to a high accuracy rate but it does not necessarily mean the model is performing well. This is because the model is not adequately identifying the minority class, which may be the class of interest.

In this case, optimizing precision may not be as important as it could lead to many false negatives, which means missing actual positive cases. However, optimizing recall and F1 score can help in identifying more true positives and minimize false negatives, which is especially important in cases where the cost of a false negative is high.

Similarly, AUC takes into account the performance of the model across a range of different threshold values, making it a useful metric for imbalanced datasets. AUC measures the ability of the model to correctly rank true positives and true negatives against false positives and false negatives, making it a good metric to evaluate the overall performance of the model on the imbalanced dataset. In this case, the model that dominates in terms of recall, F1-Score and AUC, which is the **cross-validated random forest**, is chosen as the current best model.

Although ANN is expected to dominate cross-validate random forest in terms of performance, the outcome seems to tell the opposite. This might probably due to the amount of data used to train the model is not large enough.
...
print(CM.cvrf)
```
All metrices seems indifferent compared to the non-cross-validated model.
```{r}
#Calculating Type I Error
...
After obtaining the best model, it is then applied onto the full dataset of "WAUS2020.csv" to obtain the out-of-sample accuracy of the model. The data used to train and test the model are removed to ensure the data used are new and unseen to the model.
```{r}
#Ensure the location is the same as the 10 locations we used to train
WAUS.whole = read.csv("WAUS2020.csv")
WAUS.whole <- WAUS.whole[(WAUS.whole$Location %in% L),]

#Remove variables excluded in training
col.remove <- c("Evaporation", "Sunshine", "Cloud9am", "Cloud3pm")
WAUS.whole <- WAUS.whole[, !names(WAUS.whole) %in% col.remove, drop = FALSE]

#Remove rows used in training and testing the model
trainrow <- which(rownames(WAUS.whole) %in% rownames(WAUS.train))
testrow <- which(rownames(WAUS.whole) %in% rownames(WAUS.test))

WAUS.whole <- WAUS.whole[-trainrow, ]
WAUS.whole <- WAUS.whole[-testrow, ]

#Impute Missing Values with MICE Algorithm
tempWAUS.whole <- WAUS.whole %>% mutate(
  WindGustDir = as.factor(WindGustDir),
  WindDir9am = as.factor(WindDir9am),
  WindDir3pm = as.factor(WindDir3pm),
  RainToday = as.factor(RainToday),
...
impute <- mice(tempWAUS.whole, maxit = 0)
...
#Assign polyreg for Unordered Variables and logreg.boost for Binary Variables
met[unordered] = "polyreg"
met[binary] = "logreg.boot"
met

#MICE Method to Impute Missing Values
MICE.whole <- mice(tempWAUS.whole, maxit = 5, predictorMatrix = predMat, method = met, print = FALSE)

#Export Imputed Data
comp <- complete(MICE.whole, 1)

#Check Unique Values
sapply(comp, function(y) sum(length(which(is.na(y)))))

WAUS.whole <- comp

#Predict and Produce Confusion Matrix
set.seed(100)
WAUS.whole.pred <- predict(cvrf.grid, WAUS.whole) #Prediction

CM.whole <- confusionMatrix(mode = "prec_recall", data = WAUS.whole.pred, reference = WAUS.whole$RainTomorrow) #Confusion Matrix

cat(paste("Accuracy: ", CM.whole$overall['Accuracy'], "F1: ", CM.whole$byClass['F1']))
```
In this case, the model predicts whether it will rain tomorrow with 86.55% out-of-sample accuracy and a F1-Score of 91.93%. The model generalize well to out-of-sample data and would produce a reliable prediction.
```{r}
rmarkdown::render()
```